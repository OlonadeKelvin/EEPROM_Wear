...
async def test_write_increments_total(dut):
    ...
    if not valid:
        cocotb.log.info(f"DEBUG(test_write_increments_total): uo_out=0x{int(dut.uo_out.value):02x} uio_out=0x{int(dut.uio_out.value):02x}")
    assert valid == 1, "telem_valid not asserted"
    if lo != 3:
        cocotb.log.info(f"DEBUG(test_write_increments_total): total_lo={lo}")
...
async def test_telem_valid_is_one_cycle_pulse(dut):
    ...
    await RisingEdge(dut.clk)
    if _telem_vld(dut) != 1:
        cocotb.log.info(f"DEBUG(test_telem_valid_is_one_cycle_pulse) uo_out=0x{int(dut.uo_out.value):02x} uio_out=0x{int(dut.uio_out.value):02x}")
...
async def test_telem_skew_bounded(dut):
    ...
    if not valid:
        cocotb.log.info(f"DEBUG(test_telem_skew_bounded): uo_out=0x{int(dut.uo_out.value):02x} uio_out=0x{int(dut.uio_out.value):02x}")
    assert valid == 1, "telem_valid not asserted"
    assert skew <= PSI + 1, (
...
async def test_telem_total_write_16bit(dut):
    ...
    if not (valid_lo and valid_hi):
        cocotb.log.info(f"DEBUG(test_telem_total_write_16bit): uo_out=0x{int(dut.uo_out.value):02x} uio_out=0x{int(dut.uio_out.value):02x}")
    assert valid_lo and valid_hi, "telem_valid not asserted"
    total = (hi << 8) | lo
    if total != WRITES_COUNT:
        cocotb.log.info(f"DEBUG(test_telem_total_write_16bit): hi=0x{hi:02x} lo=0x{lo:02x} total=0x{total:04x}")
    ...
    for i in range(CNT_MAX + 16):
        ...
        for _ in range(64):
            if _move_req(dut):
                ...
                await RisingEdge(dut.clk)
                if _move_req(dut):
                    cocotb.log.info(f"DEBUG(test_retirement_on_saturation): move_req still set, uo_out=0x{int(dut.uo_out.value):02x}")
                if _busy(dut):
                    cocotb.log.info(f"DEBUG(test_retirement_on_saturation): busy still set, uo_out=0x{int(dut.uo_out.value):02x}")
    ...
    for i in range(CNT_MAX + 16):
        ...
        for _ in range(64):
            if _move_req(dut):
                ...
                for hold in range(5):
                    if _move_req(dut) != 1:
                        cocotb.log.info(f"DEBUG(test_move_req_held_until_ack): move_req dropped at hold {hold}, uo_out=0x{int(dut.uo_out.value):02x}")
...
async def test_stress_random_200(dut):
    ...
    valid_hi, hi = await do_telem(dut, TSEL_TOTAL_HI)
    if not (valid_lo and valid_hi):
        cocotb.log.info(f"DEBUG(test_stress_random_200): uo_out=0x{int(dut.uo_out.value):02x} uio_out=0x{int(dut.uio_out.value):02x}")
    total = (hi << 8) | lo
    if total != (expected & 0xFFFF):
        cocotb.log.info(f"DEBUG(test_stress_random_200): expected=0x{expected & 0xFFFF:04x} got=0x{total:04x} hi=0x{hi:02x} lo=0x{lo:02x}")
